...
void script_main()
{
	...
	static auto player_god_mode = false;
	static auto player_no_ragdoll = false;
	static auto player_wanted_level = 0;
	static auto player_never_wanted = false;
	static auto player_fastrun = false;
	static auto player_super_jump = false;

	static auto vehicle_godmode = false;
	static auto vehicle_tank_godmode = false;
	static auto vehicle_super_boost = false;
	...
	origin->create_menu("main", "off", []
	{
		...
		origin->create_option(new blank_option("Online", "Comming", "Menu des joueurs."));
	...
	origin->create_menu("main_teleportation", "main", []
	{
		...
		origin->create_option(new blank_option("Mount Chiliad", ">>", "TP vers le Mount Chiliad !", [] { natives::Vector3 Coords; Coords.x = 496.75f; Coords.y = 5591.17f; Coords.z = 795.03f; Kaotic::Player::TeleportToCoords(Coords); origin->notify("Teleportation"); }));
	...
	origin->create_menu("main_test", "main", []
	{
		...
		//origin->create_option(new input_option("Set Plate", &vehicle_plate, "Custom la plaque", [] { Vehicle::SetPlate((char*)vehicle_plate.c_str()); origin->notify("Set Plate!"); }));
